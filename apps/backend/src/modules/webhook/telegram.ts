...
export const generateTelegramMessage = (disasterType: DisasterType, form: DisasterDtoType) => {
  ...
  const disasterName = get(
    Mapping,
    ['disasters', commonFields.disTyp],
    startCase(disasterType),
  ) as string;
  ...
  const commonSections: MessageSection[] = [
    {
      format: (v) =>
        `- ប៉ះពាល់: ${v[FloodSpecific.NumFamAff]} គ្រួសារ និងមនុស្ស  ${
          v[FloodSpecific.NumPeoAff]
        } នាក់`,
  ...
  const floodSections: MessageSection[] = [
    {
      format: (v) =>
        `- ជម្លៀស: ${v[FloodSpecific.NumFamEva]} គ្រួសារ និងមនុស្ស ${
          v[FloodSpecific.NumPeoEva]
        } នាក់`,
      condition: (v) => v[FloodSpecific.NumFamEva] > 0 || v[FloodSpecific.NumPeoEva] > 0,
    },
    {
      format: (v) =>
        `- មនុស្ស: ស្លាប់ ${v[FloodSpecific.NumTDeath]} នាក់ របួស ${
          v[FloodSpecific.NumTInjure]
        } នាក់ និងបាត់ខ្លួន ${v[IncidentSpecific.ToNumMising]} នាក់`,
      condition: (v) =>
        v[FloodSpecific.NumTDeath] > 0 ||
        v[FloodSpecific.NumTInjure] > 0 ||
        v[IncidentSpecific.ToNumMising] > 0,
    },
    {
      format: (v) =>
        `- ផ្ទះ: ប៉ះពាល់ ${v[FloodSpecific.NumHouAff]} ខ្នង និងខូចខាត ${
          v[FloodSpecific.NumHouDam]
        } ខ្នង`,
      condition: (v) => v[FloodSpecific.NumHouAff] > 0 || v[FloodSpecific.NumHouDam] > 0,
    },
    {
      format: (v) =>
        `- ប៉ះពាល់ផ្នែកកសិកម្ម: សន្ទូង/ពង្រោះ ${v[FloodSpecific.PaddyAff]} ហត សំណាប ${
          v[FloodSpecific.SamNabAff]
        } ហត និងដំណាំរួមផ្សំ ${v[FloodSpecific.FarmAff]} ហត`,
      condition: (v) =>
        v[FloodSpecific.PaddyAff] > 0 ||
        v[FloodSpecific.SamNabAff] > 0 ||
        v[FloodSpecific.FarmAff] > 0,
    },
    {
      format: (v) =>
        `- ប៉ះពាល់ផ្លូវថ្នល់: ផ្លូវកៅស៊ូ ${v[FloodSpecific.RubberRoAff]} ម ផ្លូវបេតុង ${
          v[FloodSpecific.ConcretAff]
        } ម និងផ្លូវគ្រួសក្រហម ${v[FloodSpecific.RuralRoAff]} ម`,
      condition: (v) =>
        v[FloodSpecific.RubberRoAff] > 0 ||
        v[FloodSpecific.ConcretAff] > 0 ||
        v[FloodSpecific.RuralRoAff] > 0,
    },
    {
      format: (v) =>
        `- ស្ពានបេតុង: ប៉ះពាល់ ${v[FloodSpecific.BridgeAff]} កន្លែង ខូចខាត ${
          v[FloodSpecific.BridgeDam]
        } កន្លែង`,
      condition: (v) => v[FloodSpecific.BridgeAff] > 0 || v[FloodSpecific.BridgeDam] > 0,
    },
    {
      format: (v) =>
        `- ស្ពានបេឡេត៍: ប៉ះពាល់ ${v[FloodSpecific.BeleBridAff]} កន្លែង ខូចខាត ${
          v[FloodSpecific.BeleBridDam]
        } កន្លែង`,
  ...
  const droughtSections: MessageSection[] = [
    {
      format: (v) =>
        `- សន្ទូង/ពង្រោះ: ប៉ះពាល់ ${v[DroughtSpecific.PaddyAff]} ហត និងខូចខាត ${
          v[DroughtSpecific.PaddyDam]
        } ហត`,
      condition: (v) => v[DroughtSpecific.PaddyAff] > 0 || v[DroughtSpecific.PaddyDam] > 0,
    },
    {
      format: (v) =>
        `- សំណាប: ប៉ះពាល់ ${v[DroughtSpecific.SamNabAff]} ហត និងខូចខាត ${
          v[DroughtSpecific.SamNabDam]
        } ហត`,
      condition: (v) => v[DroughtSpecific.SamNabAff] > 0 || v[DroughtSpecific.SamNabDam] > 0,
    },
    {
      format: (v) =>
        `- ដំណាំរួមផ្សំ: ប៉ះពាល់ ${v[DroughtSpecific.FarmAff]} ហត និងខូចខាត ${
          v[DroughtSpecific.FarmDam]
        } ហត`,
  ...
  const incidentSections: MessageSection[] = [
    {
      format: (v) =>
        `- មនុស្ស: ស្លាប់ ${v[IncidentSpecific.NumDeathTo]} នាក់ របួស ${
          v[IncidentSpecific.ToNumInjure]
        } នាក់ និងបាត់ខ្លួន ${v[IncidentSpecific.ToNumMising]} នាក់`,
      condition: (v) =>
        v[IncidentSpecific.NumDeathTo] > 0 ||
        v[IncidentSpecific.ToNumInjure] > 0 ||
        v[IncidentSpecific.ToNumMising] > 0,
    },
    // Roth had a line for houses but the fields don't seem to exist for incidents
    {
      format: (v) =>
        `- ផ្ទះ: ប៉ះពាល់ ${v[IncidentSpecific.SchAff]} ខ្នង និងខូចខាត ${
          v[IncidentSpecific.SchDam]
        } ខ្នង`,
